...
class AnimacaoMenu:
	...
	var nome_animacao_atual : String = "piscar_olho_panela"; # comeco com essa animacao...
...
@export var game_configs : Node;

@onready var animacao_configs : AnimacaoMenu = AnimacaoMenu.new();


func piscar_olho() -> void:
	if game_configs.config_ativa(game_configs.config_remover_animacao):
		return;
...
func _ready() -> void:
	...
	# coloco um valor aleatorio no timer de 2.5s ate 1s
	animacao_configs.piscar_olho_timer.start(randf() * 1.5 + 1);
	
	# quando o timer acabar ele vai chamar a funcao de piscar o olho
...
func _process(delta: float) -> void:
	...
	if not game_configs.config_ativa(game_configs.config_remover_animacao):
